...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const existing = await prisma.quote.findUnique({ where: { id }, select: { status: true, requestId: true } });
    ...
    const quote = await prisma.$transaction(async (transaction) => {
      ...
      if (!asDraft) await transaction.quoteRequest.update({ where: { id: existing.requestId }, data: { status: "QUOTED" } });
      return updated;
    });
