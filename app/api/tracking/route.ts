...
import { createTrackingToken, normalizeRut, normalizeTrackingIdentifier } from "@/lib/public-tracking";
...
export async function POST(request: Request) {
  ...
  const requestNumber = typeof payload.requestNumber === "string" ? normalizeTrackingIdentifier(payload.requestNumber) : "";
  ...
  if (shouldUseJsonStorage()) {
    const record = (await readDevQuoteRequests()).find((item) => normalizeTrackingIdentifier(item.requestNumber) === requestNumber && normalizeRut(item.requesterRut || item.customer?.rut || "") === rut);
  ...
  const record = records.find((item) => normalizeTrackingIdentifier(item.requestNumber) === requestNumber && normalizeRut(item.requesterRut) === rut);
