...
import { AlertTriangle, CheckCircle2, Loader2, RotateCcw, ShieldAlert, UserCog } from "lucide-react";
...
export default function ManageStatusModal({
  ...
  useEffect(() => {
    if (!open || (step !== "assign-select" && step !== "reactivate") || executives.length) return;
  ...
  const confirmReactivation = async () => {
    if (!reason.trim() || !selectedExecutive || isBusy) return;
    try {
      setStage("submitting");
      setError("");
      const response = await fetch(`/api/quote-requests/${requestId}/status`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "REACTIVATE", executiveId: selectedExecutive.id, reason: reason.trim() }),
      });
      const result = (await response.json()) as { error?: string } & ManageStatusResult;
      if (!response.ok) throw new Error(result.error || "No fue posible reactivar la solicitud");
      onUpdated(result);
      setStage("success");
    } catch (submitError) {
      setStage("error");
      setError(submitError instanceof Error ? submitError.message : "No fue posible reactivar la solicitud");
    }
  };
  ...
  const title =
    stage === "success"
      ? step === "reject"
        ? "Solicitud rechazada"
        : step === "reactivate-confirm" || step === "reactivate"
    ...
    <Modal
      ...
      footer={
        stage === "success" ? (
          ...
          <div className="flex justify-end gap-2">
            <SecondaryButton size="sm" onClick={() => setStep("choose")}>Volver</SecondaryButton>
            <PrimaryButton size="sm" onClick={() => setStep("reactivate-confirm")} disabled={!reason.trim() || !selectedExecutive}>Continuar</PrimaryButton>
          </div>
        ) : step === "reactivate-confirm" ? (
          <div className="flex justify-end gap-2">
            <SecondaryButton size="sm" onClick={() => setStep("reactivate")} disabled={isBusy}>Volver</SecondaryButton>
            <PrimaryButton size="sm" onClick={confirmReactivation} disabled={isBusy || !reason.trim() || !selectedExecutive} icon={isBusy ? Loader2 : undefined} className={isBusy ? "[&_svg]:animate-spin" : ""}>
              {isBusy ? "Reactivando..." : "Confirmar reactivación"}
            </PrimaryButton>
          </div>
        ) : (
          <div className="flex justify-end gap-2">
    ...
    >
      {stage === "success" ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          ...
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-blue-50 text-[var(--blue)]"><UserCog className="h-4 w-4" /></div>
            <div><p className="text-sm font-bold text-[var(--navy)]">Enviar poder / mandato</p><p className="mt-1 text-xs text-[var(--text-secondary)]">Genera el mandato y lo envía al correo registrado del cliente.</p></div>
          </button>}
          ...
          {currentStatus === "REJECTED" && <button
            type="button"
            onClick={() => setStep("reactivate")}
            className="flex w-full items-start gap-3 rounded-xl border border-[var(--border)] p-4 text-left transition hover:border-[var(--blue)] hover:bg-[var(--background)]"
          >
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><RotateCcw className="h-4 w-4" /></div>
            <div>
              <p className="text-sm font-bold text-[var(--navy)]">Reactivar solicitud</p>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-3">
          ...
          <label className="block text-xs font-bold text-[var(--navy)]">
            ...
            <textarea
              ...
              placeholder="Describe por qué se reactiva esta solicitud..."
              rows={4}
              className="field-input mt-2"
            />
          </label>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          <p className="text-sm text-[var(--text-secondary)]">Al confirmar, la solicitud pasará de Rechazada a En gestión y el ejecutivo quedará asignado.</p>
          <div className="flex items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4">
            <Avatar name={`${selectedExecutive?.firstName ?? ""} ${selectedExecutive?.lastName ?? ""}`} size="lg" />
            <div>
              <p className="text-sm font-bold text-[var(--navy)]">{selectedExecutive?.firstName} {selectedExecutive?.lastName}</p>
              <p className="text-xs text-[var(--text-secondary)]">Ejecutivo responsable</p>
            </div>
          </div>
          <div className="rounded-xl border border-[var(--border)] bg-[var(--background)] p-4">
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[var(--text-secondary)]">Motivo</p>
            <p className="mt-1 whitespace-pre-wrap text-sm text-[var(--navy)]">{reason.trim()}</p>
          </div>
          {stage === "error" && (
            <div className="flex items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 p-3">
              <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0 text-rose-600" />
