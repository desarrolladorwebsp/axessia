...
import { isMedicalDevice, productTypeLabel, productTypePluralLabel, type ProductType } from "@/lib/product-type";
...
function DetailCapsule({ icon, label, value, detail, tone = "blue" }: { icon: React.ReactNode; label: string; value: React.ReactNode; detail?: string; tone?: "blue" | "purple" | "green" | "amber" }) { const tones = { blue: "border-blue-100 bg-blue-50/70 text-[var(--blue)]", purple: "border-violet-100 bg-violet-50/70 text-[var(--purple)]", green: "border-emerald-100 bg-emerald-50/70 text-emerald-700", amber: "border-amber-100 bg-amber-50/80 text-amber-700" }; return <div className={`flex min-w-0 items-center gap-3 rounded-full border px-4 py-3 ${tones[tone]}`}><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/80">{icon}</div><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.12em] opacity-70">{label}</p><p className="truncate text-sm font-extrabold text-[var(--navy)]">{value}</p>{detail && <p className="truncate text-[10px] text-[var(--text-secondary)]">{detail}</p>}</div></div>; }
...
export default function SolicitudDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-[1480px] px-1 py-2 sm:px-2 lg:px-4"><Link href="/app/solicitudes" className="inline-flex items-center gap-2 text-xs font-bold text-[var(--blue)] transition hover:text-[var(--navy)]"><ArrowLeft className="h-4 w-4" />Volver a solicitudes</Link>
    ...
    <section className="mt-5 rounded-2xl border border-[var(--border)] bg-white p-4 shadow-[0_10px_28px_rgba(7,30,65,0.04)]"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--purple)]">Resumen operativo</p><h2 className="mt-1 font-display text-base font-extrabold text-[var(--navy)]">Información clave de esta solicitud</h2></div><span className="text-[10px] font-semibold text-[var(--text-secondary)]">Actualizado {new Date(request.updatedAt || request.createdAt).toLocaleDateString("es-CL")}</span></div><div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4"><DetailCapsule icon={<Clock3 className="h-4 w-4" />} label="Estado" value={statusLabels[request.status] || request.status} detail="situación actual" tone="purple" /><DetailCapsule icon={<UserRound className="h-4 w-4" />} label="Cliente" value={customerName} detail={customerEmail} tone="blue" /><DetailCapsule icon={<ReceiptText className="h-4 w-4" />} label="Cotizaciones" value={request.quotes.length ? `${request.quotes.length} registrada${request.quotes.length === 1 ? "" : "s"}` : "Sin cotizaciones"} detail={request.quotes.some((quote) => quote.payments?.some((payment) => payment.status === "PAID")) ? "Pago confirmado" : "Revisión pendiente"} tone={request.quotes.some((quote) => quote.payments?.some((payment) => payment.status === "PAID")) ? "green" : "amber"} /><DetailCapsule icon={<FileText className="h-4 w-4" />} label="Archivos" value={`${(request.prescription ? 1 : 0) + (request.clientDocuments?.length ?? 0)} adjunto${(request.prescription ? 1 : 0) + (request.clientDocuments?.length ?? 0) === 1 ? "" : "s"}`} detail={request.origin === "WEB" ? "Ingresada por el sitio web" : "Ingresada por ejecutivo"} tone="green" /></div></section>

    <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(19rem,0.65fr)]"><main className="space-y-5"><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4"><SummaryCard icon={<ClipboardList className="h-4 w-4" />} tone="violet" label={productTypePluralLabel(request.productType)} value={String(isMedicalDevice(request.productType) ? (request.medicalDevices ?? []).length : request.medications.length)} detail="solicitados" /><SummaryCard icon={<FileText className="h-4 w-4" />} tone="green" label={isMedicalDevice(request.productType) ? "Documento" : "Receta"} value={request.prescription ? "Sí" : "No"} detail="archivo registrado" /><SummaryCard icon={<UserRound className="h-4 w-4" />} tone="blue" label="Paciente" value={request.patientName || "No informado"} detail="datos disponibles" /><SummaryCard icon={<ReceiptText className="h-4 w-4" />} tone="cyan" label="Precio" value={request.price ? `$${Number(request.price).toLocaleString("es-CL")}` : "-"} detail="cotización" /></div>

      {isMedicalDevice(request.productType) ? (
        <Panel><SectionHeading icon={<ClipboardList className="h-4 w-4" />} title="Dispositivos médicos solicitados" detail={`${(request.medicalDevices ?? []).length} dispositivo${(request.medicalDevices ?? []).length === 1 ? "" : "s"} en esta solicitud`} /><div className="overflow-x-auto"><table className="w-full min-w-[540px] text-left"><thead><tr className="border-b border-[var(--border)] bg-[var(--background)]"><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Dispositivo</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Marca</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Modelo</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Descripción</th><th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Cantidad</th></tr></thead><tbody>{(request.medicalDevices ?? []).map((device) => <tr key={device.id} className="border-b border-[var(--border)] last:border-0"><td className="px-4 py-4 text-sm font-bold text-[var(--navy)]">{device.name}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{device.brand || "No informada"}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{device.model || "No informado"}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{device.description || "Sin descripción"}</td><td className="px-4 py-4 text-right text-sm font-bold text-[var(--navy)]">{device.quantity ?? "—"}</td></tr>)}</tbody></table></div></Panel>
      ) : (
        <Panel><SectionHeading icon={<ClipboardList className="h-4 w-4" />} title="Medicamentos solicitados" detail={`${request.medications.length} medicamento${request.medications.length === 1 ? "" : "s"} en esta solicitud`} /><div className="overflow-x-auto"><table className="w-full min-w-[640px] text-left"><thead><tr className="border-b border-[var(--border)] bg-[var(--background)]"><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Medicamento</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Principio activo</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Concentración</th><th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Observaciones</th><th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wide text-[var(--text-secondary)]">Comprimidos por caja</th></tr></thead><tbody>{request.medications.map((medication) => <tr key={medication.id} className="border-b border-[var(--border)] last:border-0"><td className="px-4 py-4 text-sm font-bold text-[var(--navy)]">{medication.commercialName}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{medication.activeIngredient}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{medication.concentration}</td><td className="px-4 py-4 text-xs text-[var(--text-secondary)]">{medication.notes || "—"}</td><td className="px-4 py-4 text-right text-sm font-bold text-[var(--navy)]">{medication.tabletQuantity ?? "—"}</td></tr>)}</tbody></table></div></Panel>
      )}
      <Panel><SectionHeading icon={<ReceiptText className="h-4 w-4" />} title="Cotizaciones" detail={request.quotes.length ? `${request.quotes.length} cotización${request.quotes.length === 1 ? "" : "es"} generada${request.quotes.length === 1 ? "" : "s"}` : "Sin cotizaciones aún"} />{request.quotes.length ? <div className="space-y-3">{request.quotes.map((quote) => { const paid = quote.payments?.some((payment) => payment.status === "PAID") ?? false; return <button key={quote.id} type="button" onClick={() => setSelectedQuote(quote)} className="flex w-full flex-wrap items-center justify-between gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4 text-left transition hover:border-[var(--blue)] hover:bg-white"><div className="min-w-0"><p className="text-sm font-extrabold text-[var(--navy)]">{quote.quoteNumber || `Borrador v${quote.version}`}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">{quote.items.length} producto{quote.items.length === 1 ? "" : "s"} · Emitida el {new Date(quote.createdAt).toLocaleDateString("es-CL")}</p></div><div className="flex items-center gap-3"><StatusBadge label={quoteStatusLabels[quote.status] || quote.status} tone={quoteStatusTones[quote.status]} />{quote.status === "ACCEPTED" && <span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ${paid ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{paid ? "PAGADA" : "PAGO PENDIENTE"}</span>}<p className="text-sm font-extrabold text-[var(--navy)]">{quote.total ? `$${Number(quote.total).toLocaleString("es-CL")}` : "Pendiente"}</p></div></button>; })}</div> : <div className="rounded-xl border border-dashed border-[var(--border)] bg-[var(--background)] p-4 text-sm text-[var(--text-secondary)]">Aún no se han generado cotizaciones para esta solicitud.</div>}</Panel>
      <Panel><SectionHeading icon={<FileText className="h-4 w-4" />} title={isMedicalDevice(request.productType) ? "Documento de respaldo" : "Receta médica"} detail="Documento adjunto a la solicitud" />{request.prescription ? <div className="flex flex-col gap-4 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-lg bg-white text-[var(--purple)]"><FileText className="h-5 w-5" /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-[var(--navy)]">{request.prescription.fileName}</p><p className="mt-1 text-xs text-[var(--text-secondary)]">{request.prescription.mimeType} · {(request.prescription.fileSize / 1024).toFixed(1)} KB</p></div></div>{request.prescription.hasStoredFile ? <a href={storedFileApiPath("prescriptions", request.prescription.id)} target="_blank" rel="noreferrer" className="rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-xs font-bold text-[var(--blue)] transition hover:border-[var(--blue)]">{isMedicalDevice(request.productType) ? "Ver documento" : "Ver receta"}</a> : <button type="button" disabled title="Archivo no disponible" className="cursor-not-allowed rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-xs font-bold text-[var(--text-secondary)] opacity-60">{isMedicalDevice(request.productType) ? "Ver documento" : "Ver receta"}</button>}</div> : <p className="rounded-xl bg-[var(--background)] p-4 text-sm text-[var(--text-secondary)]">{isMedicalDevice(request.productType) ? "No se adjuntó un documento de respaldo." : "No se adjuntó una receta médica."}</p>}</Panel>
      ...
      <Panel><SectionHeading icon={<History className="h-4 w-4" />} title="Mapa de avance y actividades" detail="Sigue la solicitud desde su recepción hasta la entrega" />{request.events.length ? <div className="space-y-3">{request.events.map((event) => <article key={event.id} className="rounded-xl border border-[var(--border)] bg-[var(--background)] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold text-[var(--navy)]">{requestEventLabel(event.eventType)}</p><time className="text-[10px] text-[var(--text-secondary)]" dateTime={event.createdAt}>{new Date(event.createdAt).toLocaleString("es-CL")}</time></div>{event.note && <p className="mt-2 whitespace-pre-wrap text-sm text-[var(--text-secondary)]">{event.note}</p>}</article>)}</div> : <div className="rounded-xl border border-dashed border-[var(--border)] p-4 text-sm text-[var(--text-secondary)]">Aún no hay actividades registradas.</div>}</Panel>
