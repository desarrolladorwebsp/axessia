...
import { ArrowLeft, CalendarDays, Check, CircleHelp, ClipboardList, Clock3, FileText, History, MessageSquare, ReceiptText, UserRound } from "lucide-react";
...
import RequestActionsModal, { type RequestActionResult } from "./RequestActionsModal";
...
export default function SolicitudDetailPage() {
  ...
  return <div className="mx-auto w-full max-w-[1480px] px-1 py-2 sm:px-2 lg:px-4"><Link href="/app/solicitudes" className="inline-flex items-center gap-2 text-xs font-bold text-[var(--blue)] transition hover:text-[var(--navy)]"><ArrowLeft className="h-4 w-4" />Volver a solicitudes</Link>
    ...
    <RequestActionsModal
      ...
      onUpdated={(result: RequestActionResult) => setRequest((current) => current ? { ...current, status: result.status, updatedAt: result.updatedAt || new Date().toISOString() } : current)}
    />
