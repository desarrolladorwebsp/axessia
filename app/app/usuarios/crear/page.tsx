...
export default function InviteUserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormData({ email: "", rut: "", role: "EJECUTIVO" });
    } catch (err) {
    ...
    <div className="mx-auto w-full max-w-2xl px-1 py-2 sm:px-2 lg:px-4">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
              {error}
            </motion.div>
          )}

          {success && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-700 flex items-center gap-2">
          ...
          <div>
            <label className="mb-2 block text-xs font-bold text-[var(--navy)]">Correo electrónico</label>
            <input
              type="email"
              name="email"
          ...
          <div>
            <label className="mb-2 block text-xs font-bold text-[var(--navy)]">RUT</label>
            <input
              type="text"
              name="rut"
              value={formData.rut}
          ...
          <div>
            <label className="mb-2 block text-xs font-bold text-[var(--navy)]">Perfil / Rol</label>
            <select
              name="role"
              value={formData.role}
              onChange={handleChange}
          ...
          <div className="flex gap-4 border-t border-[var(--border)] pt-6">
            <SecondaryButton type="button" onClick={() => router.back()} className="flex-1">Cancelar</SecondaryButton>
