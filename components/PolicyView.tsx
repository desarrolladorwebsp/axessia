...
export type PolicyViewVariant = "all" | "terms" | "privacy";

const viewCopy: Record<PolicyViewVariant, { title: string; accent: string; description: string }> = {
  all: {
    title: "Términos, condiciones y",
    accent: "políticas",
    description: "Reunimos la información que orienta el uso de nuestros canales, la gestión de solicitudes y el cuidado de los datos personales que nos confías.",
  },
  terms: {
    title: "Términos y",
    accent: "condiciones",
    description: "Condiciones de uso de los canales digitales de AXESSIA y las reglas aplicables a solicitudes, cotizaciones y gestión del servicio.",
  },
  privacy: {
    title: "Política de",
    accent: "privacidad",
    description: "Cómo AXESSIA trata los datos personales que nos confías y cuáles son tus derechos sobre esa información.",
  },
};

function sectionsFor(variant: PolicyViewVariant) {
  if (variant === "terms") return termsSections;
  if (variant === "privacy") return privacySections;
  return allSections;
}

export default function PolicyView({ variant }: { variant: PolicyViewVariant }) {
  const copy = viewCopy[variant];
  const sections = sectionsFor(variant);
  const termsHeading = sections.find((section) => section.id === "terminos");
  const comercialesHeading = sections.find((section) => section.id === "comerciales");
  const privacyHeading = sections.find((section) => section.id === "privacidad");
  const rightsHeading = sections.find((section) => section.id === "derechos");
      ...
      <section className="policy-hero" aria-labelledby="policies-title">
        <div className="policy-hero-inner">
          ...
          <h1 id="policies-title">
            {copy.title} <span className="gradient-text">{copy.accent}</span>
          </h1>
          ...
          {termsHeading && comercialesHeading && (
            <>
              ...
              <section id="comerciales" className="policy-section" aria-labelledby="comerciales-title">
                <PolicyHeading section={comercialesHeading} />
                <div className="policy-copy">
                  ...
                  <p>Los tiempos de envío informados en una cotización son estimados. AXESSIA no se responsabiliza por atrasos ocasionados por factores externos o terceros fuera del control de la empresa, incluyendo proveedores, transportistas, aduanas u otras autoridades.</p>
                </div>
              </section>
            </>
          )}

          {privacyHeading && rightsHeading && (
            <>
              <section id="privacidad" className="policy-section" aria-labelledby="privacidad-title">
                <PolicyHeading section={privacyHeading} />
                <div className="policy-copy">
              ...
              <section id="derechos" className="policy-section" aria-labelledby="derechos-title">
                <PolicyHeading section={rightsHeading} />
                <div className="policy-copy">
...
function PolicyHeading({ section }: { section: PolicySection }) {
