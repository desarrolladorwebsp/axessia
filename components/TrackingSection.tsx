...
export default function TrackingSection() {
  ...
  return <section className="tracking-section" id="seguimiento" aria-labelledby="tracking-title">
    ...
    {summary ? <motion.section className="mx-auto mt-8 max-w-5xl px-5 sm:px-8" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} aria-live="polite"><div className="card-surface rounded-2xl p-5 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Resumen protegido</p><h3 className="mt-2 text-2xl font-bold text-[var(--navy)]">Solicitud {summary.requestNumber}</h3></div><div className="tracking-current-status" aria-label={`Estado actual: ${labels[summary.status] || summary.status}`}><span>Estado actual</span><strong>{labels[summary.status] || summary.status}</strong></div></div><div className="mt-6 grid gap-4 border-y border-[var(--border)] py-4 text-sm sm:grid-cols-2"><p><strong className="block text-[var(--navy)]">Creada</strong><span className="text-[var(--text-secondary)]">{date(summary.createdAt)}</span></p><p><strong className="block text-[var(--navy)]">Última actualización</strong><span className="text-[var(--text-secondary)]">{date(summary.updatedAt)}</span></p></div><div className="mt-5"><h4 className="font-bold text-[var(--navy)]">{summary.productType === "MEDICAL_DEVICE" ? "Dispositivos solicitados" : "Medicamentos solicitados"}</h4><ul className="mt-3 grid gap-2 text-sm text-[var(--text-secondary)]">{summary.productType === "MEDICAL_DEVICE" ? (summary.medicalDevices ?? []).map((device) => <li key={`${device.name}-${device.model ?? "sin-modelo"}`} className="flex justify-between gap-4 border-b border-[var(--border)] py-2"><span>{device.name}{device.brand ? ` · ${device.brand}` : ""}</span><strong>{device.quantity != null ? `${device.quantity} unidades` : "Cantidad no informada"}</strong></li>) : summary.medications.map((medication) => <li key={`${medication.commercialName}-${medication.concentration}`} className="flex justify-between gap-4 border-b border-[var(--border)] py-2"><span>{medication.commercialName} · {medication.concentration}</span><strong>{medication.tabletQuantity != null ? `${medication.tabletQuantity} unidades` : "Cantidad no informada"}</strong></li>)}</ul></div>{summary.hasQuote || ["ACCEPTED", "AWAITING_DECISION"].includes(summary.status) ? <a className="brand-gradient mt-6 inline-flex items-center gap-2 rounded-full px-5 py-3 text-sm font-bold text-white" href={`/seguimiento/${encodeURIComponent(summary.requestNumber)}`}>{summary.status === "ACCEPTED" ? "Ver cotización y continuar" : "Ver cotización"} <ArrowRight size={17} aria-hidden="true" /></a> : <p className="mt-6 text-sm font-semibold text-[var(--text-secondary)]">Tu solicitud continúa en revisión. Te mostraremos la cotización cuando esté disponible.</p>}</div></motion.section> : <><div className="tracking-process-head"><span className="brand-gradient" /><h3>Así avanza tu solicitud</h3><span className="brand-gradient" /></div><motion.div className="tracking-stages" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.15 }} variants={{ visible: { transition: { staggerChildren: 0.08 } } }}>{stages.map(([number, label, title]) => <motion.article className="tracking-stage" key={number} variants={{ hidden: { opacity: 0, y: 22 }, visible: { opacity: 1, y: 0 } }}><div className="tracking-stage-heading"><span>{number}</span><strong>{label}</strong></div><div className="tracking-stage-line"><i className={number === "01" ? "is-complete" : ""} /></div><p>{title}</p><small>Según avance de tu solicitud</small></motion.article>)}</motion.div></>}
  </section>;
}
